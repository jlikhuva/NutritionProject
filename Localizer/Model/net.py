...
class LocalizerNet(nn.Module):
    ...
    def _run_block_one(self, x):
        out = self.mp1(F.leaky_relu(self.bn1(self.conv1(x))))
        out = self.mp2(F.leaky_relu(self.bn2(self.conv2(out))))
        out = F.leaky_relu(self.bn3(self.conv3(out)))
        out = F.leaky_relu(self.bn4(self.conv4(out)))
        out = self.mp5(F.leaky_relu(self.bn5(self.conv5(out))))
        return out

    def _run_block_two(self, x):
        out = F.leaky_relu(self.bn6(self.conv6(x)))
        out = F.leaky_relu(self.bn7(self.conv7(out)))
        out = self.mp8(F.leaky_relu(self.bn8(self.conv8(out))))
        out = F.leaky_relu(self.bn9(self.conv9(out)))
        out = F.leaky_relu(self.bn10(self.conv10(out)))
        out = F.leaky_relu(self.bn11(self.conv11(out)))
        out = F.leaky_relu(self.bn12(self.conv12(out)))
        out = self.mp13(F.leaky_relu(self.bn13(self.conv13(out))))
